#!/usr/bin/env python3
"""Evaluate mesh-to-CAD reconstruction quality on all chess queen models.

Runs the full reconstruct_cad pipeline on each queen, measures quality,
and reports results.  Used to develop and refine CAD strategies for
crowned/ornate shapes alongside the chess_kings benchmark.

Usage:
    python -m dev_models.chess_queens.evaluate_queens          # all queens
    python -m dev_models.chess_queens.evaluate_queens staunton # one queen
"""
...
import numpy as np
from dev_models.chess_queens import load_queen, load_all_queens, list_queens
...
def evaluate_single(name, target_v, target_f, out_dir=None):
    """Evaluate reconstruction for a single queen.
...
def evaluate_all(names=None, save=True):
    """Evaluate all (or specified) queens.
    ...
    results = []
    print(f"Evaluating {len(names)} chess queen models...")
    ...
    for i, name in enumerate(names, 1):
        try:
            v, f = load_queen(name, normalize=True)
